...
<?php
	...
	for ($x = 1; $x <=$tot; $x++)
	{
		$b="SELECT username from users WHERE id=$x AND catagory=\"Teacher\"";
		...
		$imgnm="SELECT imgname from users WHERE id=$x AND catagory=\"Teacher\"";
	...
	function redirect(cooknm){
		...
		window.location.assign(\"tprofile_protect.php\")
	}
</script>";
	}
	}
