...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        $sql = "INSERT INTO student ( username,email,links,hobby,intrest,srn) VALUES (?,?,?,?,?,?)";
        ...
        if(   $stmt = mysqli_prepare($link, $sql) ){
            ...
            // Bind variables to the prepared statement as parameters
            if(mysqli_stmt_bind_param($stmt, "ssssss", $param_username,$param_email,$param_link,$param_hobby ,$param_intrest,$param_srn))
			{
			}
			else{
			echo ("error");}
		...
		$param_intrest=trim($_POST["intrest"]);
		$param_hobby=trim($_POST["hobby"]);
	...
	<div class="sigcon">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
			...
			<div class="form-group <?php echo (!empty($password_err)) ? 'has-error' : ''; ?>">
                ...
                <input type="text" name="intrest" class="form-control" >
                <span class="help-block"><?php echo $password_err; ?></span>
            </div>
